...
async def submit_response(body: ResponseSubmit, user: dict = Depends(current_user)):
    ...
    pacing = analyze_pacing(body.transcript, body.duration_seconds)
...
async def feed(
    ...
    db = get_db()
    sort_spec = [("created_at", -1)] if sort == "new" else [("like_count", -1), ("created_at", -1)]
    cursor = db.responses.find({"is_public": True}).sort(sort_spec).limit(limit)
    out = []
...
async def get_response(
    ...
    my_rating = None
    if user is not None:
        react = await db.reactions.find_one(
            {
...
# ---------- Reactions (like/dislike on responses AND comments) ----------
async def _apply_reaction(
    db, target_type: str, target_id: str, user_id: str, value: int
):
    ...
    # Apply delta to target counts.
    coll = db.responses if target_type == "response" else db.comments
    inc = {}
    if prev == 1:
    ...
    if value == 1:
        ...
        inc["dislike_count"] = inc.get("dislike_count", 0) + 1
    if inc:
        await coll.update_one({"_id": ObjectId(target_id)}, {"$inc": inc})


@router.post("/{response_id}/react")
async def react_response(
    response_id: str, body: ReactionCreate, user: dict = Depends(current_user)
):
    db = get_db()
    r = await db.responses.find_one({"_id": ObjectId(response_id)})
    if not r or not r.get("is_public"):
        raise HTTPException(404, "Not found")
    await _apply_reaction(db, "response", response_id, user["_id"], body.value)
    doc = await db.responses.find_one({"_id": ObjectId(response_id)})
    return {
...
async def react_comment(
    ...
    if not c:
        raise HTTPException(404, "Not found")
    await _apply_reaction(db, "comment", comment_id, user["_id"], body.value)
    # Karma for the comment author = net likes across all their comments.
